    (async function () {
      ...
      // ─── BASE PATH ───────────────────────────────────────────────
      // Standalone collection pages live in a subfolder (/eternos/, /unicos/…)
      // and set window.PARKET_BASE = '../' so relative local assets (collection
      // photos, textures) resolve from the site root. Empty on root pages.
      ...
      const ALL_PRODUCTS = [
        {
          ...
          collections: [
            ...
            },
            {
              ...
              images: [
                { src: "pisos/listone-giordano/01.webp", name: "Carvalho Europeu Smoke + Travertino Navona" },
                { src: "pisos/listone-giordano/02.webp", name: "Carvalho Europeu<br>all black + Carrara" },
                { src: "pisos/listone-giordano/03.webp", name: "Nogueira +<br>Nero Marquina + Latão" },
                { src: "pisos/listone-giordano/04.webp", name: "Carvalho Europeu Naturalle + Travertino Navona" },
                { src: "pisos/listone-giordano/05.webp", name: "Nogueira + Latão" },
                { src: "pisos/listone-giordano/06.webp", name: "Carvalho Europeu Armani com paginação petala" },
                { src: "pisos/listone-giordano/07.webp", name: "Nogueira + Nero Marquina<br>+ Latão" }
              ]
            }
          ]
        },
      ...
      async function findLocalImages(folder) {
        ...
        for (let i = 1; i <= MAX_LOCAL; i++) {
          ...
          try {
            const src = await probe(`${BASE}${folder}/${num}.webp`);
            found.push(src);
            misses = 0;
          } catch {
            if (++misses >= 2) break;
      ...
      (function texturasLaterais() {
        ...
        const ESPECIES = [
          ...
          ['nogueira', 'Nogueira', 'Escura e aveludada, de veios profundos em castanho acinzentado, imprime sobriedade e um ar classicamente nobre ao ambiente.'],
        ];
